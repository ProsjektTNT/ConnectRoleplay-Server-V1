...
RegisterNetEvent('_chat:messageEnthouseRobberies:createHouseered')
...
RegisterNUICallback('chatResult', function(data, cb)
  ...
  if not data.canceled then
    ...
    if data.message:sub(1, 1) == '/' then
      ...
      TriggerServerEvent('_chat:messageEnthouseRobberies:createHouseered', GetPlayerName(id), { r, g, b }, data.message)
    end
  end
...
RegisterNetEvent("xz:interact:init:" .. resourceName, function(Nevo)
    Nevo.Mapping:Listen("chat", "(GUI) Chat", "keyboard", "T", function(state)
        if state then
          chatInputActive = true
          SetNuiFocus(chatInputActive, chatInputActive)
