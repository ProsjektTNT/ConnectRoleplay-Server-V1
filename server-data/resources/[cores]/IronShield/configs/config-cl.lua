...
--[[Anti GodMode]]--
DiamondC.AntiGodMode = false -- Anti godmode
DiamondC.ArmourCheck = false -- Anti armor
...
DiamondC.AntiGodModeBan = false -- Ban the AntiGodMode detected player

--[[Armour Config]]--
DiamondC.MaxPlayerArmour = 200 -- Max player armor (if over, ban them)

--[[Spectate Config]]--
DiamondC.AntiSpectate = false -- Detects if a player has no permission to use

--[[Basic]]--
DiamondC.Basic = true -- (Includes infinite ammo, max player speed, etc..)
DiamondC.AntiNoclip = false -- Detect if a player try to use noclip (in testing)
DiamondC.AntiBlips = false -- Detect if  a user that activated Player Blips and is not allowed

--[KeyBlacklist]--
DiamondC.SpamWebhook = 'https://discord.com/api/webhooks/862985622288203787/WDIeeVoGp9hlOZke6My18g_40z9S200CSf4W9K-8LJHRcNZylI1wR0Kn1rnoXzPBTQ7x' -- Put your 'spam' discord webhook (for take screenshots, not for log)
...
DiamondC.AntiResourceStop = true -- Detects if a player tries to block the anticheat
DiamondC.AntiResourceStart = true -- Detects if a player tries to start another script
...
--[[Anti CheatEngine]]--
DiamondC.AntiVehicleHashChanger = false -- Detects if a player tried to change his vehicle hash model
DiamondC.AntiDamageModifier = false -- Detects if a player tries to modify his damage or defence value
DiamondC.AntiWeaponManipulator = false -- MasterSwitch for weapon damange modifier
DiamondC.AntiWeaponDamageModifier = false -- Detects if a player tried to modify the weapon damage
DiamondC.AntiExplosiveWeapons = false -- Detects if a player modified his weapon to shoot explosive bullets, or explosive punch
...
DiamondC.AntiModelChanger = false -- Detects if a player tried to change model to a blacklisted model
...
DiamondC.BlacklistWeapon = true -- Detect if a player has a blacklist weapon and remove
   DiamondC.DeleteAllWeapon = false -- (If = true, delete all player's weapons. If = true, just delete blacklist weapon)
