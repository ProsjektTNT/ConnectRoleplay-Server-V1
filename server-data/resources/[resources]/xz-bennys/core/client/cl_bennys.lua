...
local bennyLocation2 = vector3(-34.57794, -1053.609, 27.790122)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("xz-bennys:attemptPurchase", type, upgradeLevel)
...
RegisterNetEvent('enter:bennyss', function()
    TriggerEvent('event:control:bennyss', 1)
end)

RegisterNetEvent('enter:bennys2', function()
...
RegisterNetEvent('event:control:bennyss', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
...
RegisterNetEvent('event:control:bennys2', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyHeading = 30.79
        if useID == 1 and not isPlyInBennys then -- Bennys
...
exports("isInBennys",function()
    local plyPed = PlayerPedId()
    local plyPos = GetEntityCoords(plyPed)
        if isNear(plyPos, bennyLocation, 12) then 
            return true
        end
...
exports("isInBennys2",function()
    local plyPed = PlayerPedId()
    local plyPos = GetEntityCoords(plyPed)
        if isNear(plyPos, bennyLocation2, 12) then 
            return true
        end
...
exports("isInBilys",function()
    local plyPed = PlayerPedId()
    local plyPos = GetEntityCoords(plyPed)
        if isNear(plyPos, bennyPaletoLocation, 5) then 
            return true
        end
...
--#[Event Handlers]#--
RegisterNetEvent("xz-bennys:purchaseSuccessful")
AddEventHandler("xz-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    XZCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("xz-bennys:purchaseFailed")
AddEventHandler("xz-bennys:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    XZCore.Functions.Notify("Not enough money", "error")
end)