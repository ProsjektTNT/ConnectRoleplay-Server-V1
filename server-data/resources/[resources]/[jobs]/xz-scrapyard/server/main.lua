...
RegisterServerEvent('xz-scrapyard:server:LoadVehicleList')
AddEventHandler('xz-scrapyard:server:LoadVehicleList', function()
    local src = source
    TriggerClientEvent("xz-scapyard:client:setNewVehicles", src, Config.CurrentVehicles)
end)


RegisterServerEvent('xz-scrapyard:server:ScrapVehicle')
AddEventHandler('xz-scrapyard:server:ScrapVehicle', function(listKey)
    local src = source 
    ...
    if Config.CurrentVehicles[listKey] ~= nil then 
        ...
        TriggerClientEvent("xz-scapyard:client:setNewVehicles", -1, Config.CurrentVehicles)
    end
end)

function GenerateVehicleList()
    --print("Functions has been turned on")
    Config.CurrentVehicles = {}
    for i = 1, 40, 1 do
        local randVehicle = Config.Vehicles[math.random(1, #Config.Vehicles)]
        if not IsInList(randVehicle) then
            --print("Added " .. randVehicle)
