...
AddEventHandler("XZCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("xz-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										XZCore.Functions.Notify("This Vehicle Cannot Be Scrapped.", "error")
									end
								else
									XZCore.Functions.Notify("You Are Not The Driver", "error")
								end
							end
						end
					end
...
RegisterNetEvent('xz-scapyard:client:setNewVehicles')
AddEventHandler('xz-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = XZCore.Shared.Vehicles[GetHashKey(v)]
		...
		XZCore.Functions.Notify("You Are Not Allowed To Demolish Vehicles Now", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	XZCore.Functions.Progressbar("scrap_vehicle", "Demolish Vehicle", scrapTime, false, true, {
		...
		TriggerServerEvent("xz-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
