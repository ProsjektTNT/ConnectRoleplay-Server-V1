...
Citizen.CreateThread(function()
    for k, station in pairs(Config.Locations["prison"]) do
        local blip = AddBlipForCoord(station.coords.x, station.coords.y, station.coords.z)
...
Citizen.CreateThread(function()
    ...
    isHandcuffed = false
    TriggerServerEvent("XZCore:Server:SetMetaData", "ishandcuffed", false)
    TriggerServerEvent("police:server:SetHandcuffStatus", false)
    TriggerServerEvent("police:server:UpdateBlips")
    TriggerServerEvent("police:server:UpdateCurrentCops")

    if XZCore.Functions.GetPlayerData().metadata["tracker"] then
        local trackerClothingData = {outfitData = {["accessory"] = { item = 13, texture = 0}}}
        TriggerEvent('xz-clothing:client:loadOutfit', trackerClothingData)
    else
...
AddEventHandler('police:client:SendPoliceEmergencyAlert', function()
    ...
    TriggerServerEvent("police:server:SendPoliceEmergencyAlert", streetLabel, pos, XZCore.Functions.GetPlayerData().metadata["callsign"])
    TriggerServerEvent('xz-policealerts:server:AddPoliceAlert', {
        timeOut = 10000,
        ...
        details = {
            [1] = {
                ...
                detail = MyId .. ' | ' .. XZCore.Functions.GetPlayerData().charinfo.firstname .. ' ' .. XZCore.Functions.GetPlayerData().charinfo.lastname,
            },
            [2] = {
        ...
        callSign = XZCore.Functions.GetPlayerData().metadata["callsign"],
    }, true)
end)
...
AddEventHandler('police:client:VehicleCall', function(pos, alertTitle, streetLabel, modelPlate, modelName)
    if PlayerJob.name == 'police' and onDuty then
        TriggerEvent('xz-policealerts:client:AddPoliceAlert', {
            timeOut = 4000,
            alertTitle = alertTitle,
            coords = {
                x = pos.x,
