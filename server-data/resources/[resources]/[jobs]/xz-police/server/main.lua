...
AddEventHandler('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local CuffedPlayer = XZCore.Functions.GetPlayer(playerId)
    if CuffedPlayer ~= nil then
        if Player.Functions.GetItemByName("handcuffs") ~= nil or Player.PlayerData.job.name == "police" then
            ...
            TriggerClientEvent('XZCore:Notify', src, "You dont have handcuffs and you are not officer.", "error")
        end
    end
end)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = XZCore.Functions.GetPlayer(source)
    local EscortPlayer = XZCore.Functions.GetPlayer(playerId)
...
XZCore.Commands.Add("fine", "Bill Player", {{name="id", help="Player ID"}, {name="amount", help="Fine Amount"}, {name="title", help="Fine Title"}}, false, function(source, args)
    local src = source
    local Player = XZCore.Functions.GetPlayer(src)
    local OtherPlayer = XZCore.Functions.GetPlayer(tonumber(args[1]))
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == 'ambulance' or Player.PlayerData.job.name == 'mechanic' or Player.PlayerData.job.name == 'cardealer' then
        if OtherPlayer ~= nil then
            if price and price > 0 then
                ...
                exports.oxmysql:insert('INSERT INTO `phone_invoices` (`citizenid`, `amount`, `society`, `title`) VALUES (?, ?, ?, ?)', { OtherPlayer.PlayerData.citizenid, price, Player.PlayerData.job.name, title }, function()
                    TriggerClientEvent('XZCore:Notify', OtherPlayer.PlayerData.source, "You received a fine for a total of $" .. price)
                    TriggerClientEvent("xz-phone:RefreshPhone", OtherPlayer.PlayerData.source)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('XZCore:Notify', src, "You sent the person to prison for "..time.." months")
        end
    end
end)
...
XZCore.Functions.CreateCallback('police:server:GetCuffedStatus', function(source, cb, playersrc)
    local Player = XZCore.Functions.GetPlayer(playersrc)
...
AddEventHandler('police:server:VehicleCall', function(pos, msg, alertTitle, streetLabel, modelPlate, modelName)
    ...
    TriggerClientEvent("xz-phone:client:addPoliceAlert", -1, alertData)
end)
...
AddEventHandler('police:server:SendEmergencyMessage', function(coords, message)
    local src = source
    local MainPlayer = XZCore.Functions.GetPlayer(src)
    local alertData = {
...
AddEventHandler('police:server:checkBank', function(playerId)
    local src = source
    local SearchedPlayer = XZCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:checkFines', function(playerId)
    ...
    local SearchedPlayer = XZCore.Functions.GetPlayer(playerId)
    if SearchedPlayer ~= nil then 
        exports.oxmysql:fetch('SELECT * FROM `phone_invoices` WHERE `citizenid` = ? AND `society` = "police"', { SearchedPlayer.PlayerData.citizenid }, function(invoices)
            if #invoices == 0 then
                TriggerClientEvent('chat:addMessage', src, {
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('XZCore:Notify', SearchedPlayer.PlayerData.source, "You are being searched..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('XZCore:Notify', SearchedPlayer.PlayerData.source, "Your cash confiscated..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('XZCore:Notify', SearchedPlayer.PlayerData.source, "Your driving license confiscated..")
        else
            TriggerClientEvent('XZCore:Notify', src, "Can't take driving license..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('XZCore:Notify', SearchedPlayer.PlayerData.source, "You're like €"..money.." robbed")
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            exports['oxmysql']:execute('UPDATE player_vehicles SET state = @state, depotprice = @depotprice WHERE plate = @plate', {['@state'] = 0, ['@depotprice'] = price, ['@plate'] = plate})
            TriggerClientEvent('XZCore:Notify', src, "Vehicle included in depot for €"..price.."!")
        else
            exports['oxmysql']:execute('UPDATE player_vehicles SET state = @state WHERE plate = @plate', {['@state'] = 2, ['@plate'] = plate})
            TriggerClientEvent('XZCore:Notify', src, "Vehicle completely seized!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, bloodInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, XZCore.Shared.Items["filled_evidence_bag"], "add")
            TriggerClientEvent("evidence:client:RemoveBlooddrop", -1, bloodId)
            BloodDrops[bloodId] = nil
        end
    else
        TriggerClientEvent('XZCore:Notify', src, "You must have an empty evidence bag with you", "error")
    end
end)
...
AddEventHandler('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = XZCore.Shared.Weapons[weapon]
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('XZCore:Notify', targetId, 'Your ankle strap is taken off.', 'error', 5000)
        TriggerClientEvent('XZCore:Notify', source, 'You took off an ankle bracelet from '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('XZCore:Notify', targetId, 'You got an ankle bracelet.', 'error', 5000)
        TriggerClientEvent('XZCore:Notify', source, 'You put on an ankle strap at '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = XZCore.Functions.GetPlayer(source)
    local TrackerMeta = Target.PlayerData.metadata["tracker"]
    ...
    TriggerClientEvent("xz-phone:client:addPoliceAlert", requestId, alertData)
end)
...
XZCore.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = XZCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"] or Player.PlayerData.metadata["inlaststand"])
end)

XZCore.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = XZCore.Functions.GetPlayer(playerId)
    local statList = {}
	if Player ~= nil then
...
XZCore.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = XZCore.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(XZCore.Shared.Weapons[weapon]["name"])
    local retval = false
...
XZCore.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    for k, v in pairs(XZCore.Functions.GetPlayers()) do
        local Player = XZCore.Functions.GetPlayer(v)
        if Player ~= nil then 
...
XZCore.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
XZCore.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
XZCore.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
...
XZCore.Commands.Add("spike", "Place down a spike strip", {}, false, function(source, args)
    local Player = XZCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
...
XZCore.Commands.Add("pobj", "Place / Delete an object", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    local Player = XZCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('XZCore:Notify', source, "This command is for cops only!", "error")
    end
end)

XZCore.Commands.Add("pdv", "Delete a vehicle | Police Only", {}, false, function(source, args)
    local src = source
    local Player = XZCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" then
        TriggerClientEvent('XZCore:Command:DeleteVehicle', src)
        TriggerEvent('xz-logs:server:createLog', 'emergency', 'Command "pdv"', "Used the command **pdv**", source)
    else
        TriggerClientEvent('chat:addMessage', source , {
            template = '<div class="chat-message emergency"><b>SYSTEM:</b> {0}</div>',
            args = { "This command is for cops only!" }
        })
    end
end)

XZCore.Commands.Add("cuff", "Cuff citizen", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("seatvehicle", "Put player in vehicle", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("unseatvehicle", "Get player out from vehicle", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("escort", "Escort citizen", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    TriggerClientEvent("police:client:EscortPlayer", source)
end)

XZCore.Commands.Add("clearcasings", "Take away bullets in the area (make sure you have picked up some)", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("jail", "Jail person.", {{name="id", help="Player ID"},{name="time", help="Time"}}, true, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("unjail", "Unjail person.", {{name="id", help="Player ID"}}, true, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
...
XZCore.Commands.Add("clearblood", "Take away nearby blood (make sure you've picked up some)", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("seizecash", "Take cash from the nearest person", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
XZCore.Commands.Add("softcuff", "Handcuff someone but let him walk", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("camera", "View security camera", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("impound", "Send a vehicle to the impound", {{name="price", help="Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Commands.Add("fullimpound", "Seize a vehicle", {}, false, function(source, args)
	local Player = XZCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XZCore.Functions.CreateUseableItem("handcuffs", function(source, item)
    local Player = XZCore.Functions.GetPlayer(source)
...
XZCore.Commands.Add("deadofficer", "Send a message back to a notification", {}, false, function(source, args)
    local Player = XZCore.Functions.GetPlayer(source)
...
XZCore.Commands.Add("tds", "Confiscate driver license", {}, false, function(source, args)
    local Player = XZCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) then
...
XZCore.Commands.Add("tdna", "Take a DNA copy from a person (empty proof bag required)", {{"id", "Person ID"}}, true, function(source, args)
    local Player = XZCore.Functions.GetPlayer(source)
    local OtherPlayer = XZCore.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", source, XZCore.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('XZCore:Notify', source, "You must have an empty proof bag with you", "error")
        end
    end
end)
...
XZCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    for k, v in pairs(XZCore.Functions.GetPlayers()) do
        local Player = XZCore.Functions.GetPlayer(v)
        if Player ~= nil then 
...
XZCore.Commands.Add('sl', 'Use shit lockpick', {}, false, function(source, args)
