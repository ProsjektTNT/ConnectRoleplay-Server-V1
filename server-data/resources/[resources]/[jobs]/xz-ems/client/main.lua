...
RegisterNetEvent("xz-carry:beingCarried")
AddEventHandler("xz-carry:beingCarried", function(bool)
    inCarry = bool
end)
...
RegisterNetEvent('xz-ems:client:OpenPharmacy', function()
        ...
        if XZCore ~= nil then
            ...
            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, true) < 1.5) then
                ...
                if IsControlJustReleased(0, Keys["E"]) then
                    if doctorCount >= Config.MinimalDoctors then
                        TriggerEvent("XZCore:Notify", "The report was sent to the doctors!", "success")
                        TriggerEvent("InteractSound_CL:PlayOnOne", v, "demo", 0.4)
                        TriggerServerEvent("hospital:server:SendDoctorAlert")
                    else

                        exports['xz-scripts']:Release()
                        ...
                        XZCore.Functions.Progressbar("hospital_checkin", "Checking In", 6000, false, true, {
                            ...
                            XZCore.Functions.TriggerCallback('hospital:getCheckin', function(can)
                                if can then
                                    local bedId = GetAvailableBed()
                                    if bedId ~= nil then 
                                        TriggerServerEvent("hospital:server:SendToBed", bedId, true)
                                    else
                                        XZCore.Functions.Notify("Beds occupied..", "error")
                                    end
                                else
                                    XZCore.Functions.Notify("You can't afford this medical treatment.", "error")
                                end
                            end)

                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            XZCore.Functions.Notify("Not checked in!", "error")
                        end)
                    end
                end
...
function IsAbuse()
    return exports['xz-scripts']:IsBusy() or isEscorted
end
...
AddEventHandler('hospital:client:Revive', function()
    ...
    TriggerServerEvent('xz-hud:Server:RelieveStress', 100)
    TriggerServerEvent("hospital:server:SetDeathStatus", false)
    TriggerServerEvent("hospital:server:SetLaststandStatus", false)

    TriggerServerEvent("XZCore:Server:SetMetaData", "thirst", 200)
    TriggerServerEvent("XZCore:Server:SetMetaData", "hunger", 200)
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    XZCore.Functions.Notify("Your wounds have been helped!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        ...
        local player = PlayerPedId()
        if isRevive then
            XZCore.Functions.Notify("You will be helped..")
...
AddEventHandler('hospital:removeMoney', function(source)
    local src = source
    local source = XZCore.Functions.GetPlayer(src)
...
function selectOutfit(oData)
    ...
    XZCore.Functions.Notify(oData.outfitname.." gekozen", "success", 2500)
    closeMenuFull()
    changeOutfit()
end
...
function removeOutfit(oData)
    ...
    XZCore.Functions.Notify(oData.outfitname.." is verwijderd", "success", 2500)
    closeMenuFull()
end
