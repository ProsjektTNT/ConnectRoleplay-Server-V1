        ...
        CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    --XZCore.Functions.Notify("You have bled out..", "error")
                    SetLaststand(false)
                    ...
                    local weaponLabel = XZCore.Shared.Weapons?[killerWeapon]?["label"] or "Unknown"
                    local weaponName = XZCore.Shared.Weapons?[killerWeapon]?["name"] or "Unknown_Weapon"
                    TriggerServerEvent("xz-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has killed  ".. GetPlayerName(player) .." with a **".. weaponLabel .. "** (" .. weaponName .. ")")
...
RegisterNetEvent('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        XZCore.Functions.Notify('Action impossible!', 'error')
    end
end)
...
RegisterNetEvent('hospital:client:HelpPerson', function(targetId)
    ...
    XZCore.Functions.Progressbar("hospital_revive", "Reviving person..", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        XZCore.Functions.Notify("You revived a person.")
