...
Citizen.CreateThread(function()
    for k, v in pairs(Config.Vehicles) do
        menuItem[#menuItem + 1] = {
            ...
            event = "xz-ems:client:takeOutVehicle",
            eventType = "client",
            args = {
                ["model"] = k
            },
...
RegisterNetEvent('xz-ems:client:toggleDuty', function()
    onDuty = not onDuty
    TriggerServerEvent("XZCore:ToggleDuty")
    TriggerServerEvent("police:server:UpdateBlips")
end)

RegisterNetEvent('xz-ems:client:openStash', function()
    if onDuty then
        TriggerEvent("InteractSound_CL:PlayOnOne","zipper",0.5)
        TriggerServerEvent("inventory:server:OpenInventory", "stash", "emsstash_"..XZCore.Functions.GetPlayerData().citizenid)
        TriggerEvent("inventory:client:SetCurrentStash", "emsstash_"..XZCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('xz-ems:client:openBossMenu', function()
    if onDuty then
        TriggerServerEvent("xz-bossmenu:server:openMenu")
    end
end)

RegisterNetEvent('xz-ems:client:openArmory', function()
    if onDuty then
        TriggerServerEvent("inventory:server:OpenInventory", "shop", "hospital", Config.Items)
    end
end)

RegisterNetEvent('xz-ems:client:toRoof', function()
...
RegisterNetEvent('xz-ems:client:toMain', function()
    Progressbar(10000,"Waiting For Elevator")
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
...
RegisterNetEvent('xz-ems:client:toBottom', function()
    Progressbar(10000,"Waiting For Elevator")
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.Locations["duty"]) do
        exports['xz-interact']:AddBoxZone("emsduty"..k, vector3(v.x, v.y, v.z), v.l, v.w, {
            name = "emsduty"..k,
            heading = v.h,
            debugPoly = false,
            minZ = v.minZ,
            ...
            options = {
                {
                    event = 'xz-ems:client:toggleDuty',
    ...
    for k, v in pairs(Config.Locations["stash"]) do
        exports['xz-interact']:AddBoxZone("emsstash"..k, vector3(v.x, v.y, v.z), v.l, v.w, {
            name = "emsstash"..k,
            heading = v.h,
            debugPoly = false,
            minZ = v.minZ,
    ...
    for k, v in pairs(Config.Locations["boss"]) do
        exports['xz-interact']:AddBoxZone("emsbossmenu"..k, vector3(v.x, v.y, v.z), v.l, v.w, {
            name = "emsbossmenu"..k,
            heading = v.h,
            debugPoly = false,
            minZ = v.minZ,
            ...
            options = {
                {
                    event = 'xz-ems:client:openBossMenu',
    ...
    for k, v in pairs(Config.Locations["armory"]) do
        exports['xz-interact']:AddBoxZone("emsarmory"..k, vector3(v.x, v.y, v.z), v.l, v.w, {
            name = "emsarmory"..k,
            heading = v.h,
            debugPoly = false,
            minZ = v.minZ,
            ...
            options = {
                {
                    event = 'xz-ems:client:openArmory',
                    icon = 'far fa-clipboard',
                    label = 'Hospital Safe',
                    job = {['ambulance'] = 0, ['doctor'] = 0},
                }
    ...
    exports['xz-interact']:AddBoxZone('emsclothingoutfits', vector3(300.2009, -598.8107, 43.27745), 1.5, 2.5, {
    ...
    exports['xz-interact']:AddBoxZone("emselevatormain", vector3(332.45095, -595.6943, 43.284053), 0.7, 2.5, {
        ...
        options = {
            {
                event = 'xz-ems:client:toRoof',
                icon = 'fas fa-arrow-circle-up',
                label = 'Elevator to roof',
            },
            {
                event = 'xz-ems:client:toBottom',
    ...
    exports['xz-interact']:AddBoxZone("emselevatorroof", vector3(338.54479, -583.7456, 74.161705), 0.7, 2.8, {
    ...
    exports['xz-interact']:AddBoxZone("emselevatorroof", vector3(345.7102, -580.8406, 28.797988), 0.2, 0.2, {
        name = "emselevatorroof",
        heading = 250.0,
        debugPoly = false,
        minZ = 29.0,
...
AddEventHandler("ems:stash", function()
    if PlayerJob.name ~= 'ambulance' then return XZCore.Functions.Notify("You're not EMS lmfao..", "error") end
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "emsstash_"..XZCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "emsstash_"..XZCore.Functions.GetPlayerData().citizenid)
end)
...
AddEventHandler('hospital:client:AiCall', function()
    ...
    local gender = XZCore.Functions.GetPlayerData().gender
    ...
    XZCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerJob.name == "ambulance" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                XZCore.Functions.Progressbar("hospital_revive", "Help person up..", 5000, false, true, {
                    ...
                    XZCore.Functions.Notify("You helped the person!")
    ...
    XZCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "doctor" or PlayerData.job.name == "ambulance" then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                XZCore.Functions.Progressbar("hospital_heal", "Healing", 6000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
...
RegisterNetEvent("xz-ems:client:takeOutVehicle", function(data)
...
function Progressbar(duration, label)
	local retval = nil
	XZCore.Functions.Progressbar("ems", label, duration, false, false, {
