...
Citizen.CreateThread(function()
    ...
    while XZCore.Functions.GetPlayerData().job == nil do Wait(10) end
end)
...
AddEventHandler('truckrobbery:AttemptHeist', function(veh)
    ...
    XZCore.Functions.Progressbar("unlockdoor_action", "Unlocking Vehicle",
                                 45000, false, true, {
...
function pickUpCash()
    if not pickingup then
        ...
        while pickingup do
            ...
            for i = 1, #Truckrobbery.Items, 1 do
                Wait(3000)
                TriggerServerEvent('truckrobbery:addItem', Truckrobbery.Items[i].name, Truckrobbery.Items[i].count)
            end
            TriggerServerEvent('truckrobbery:addMoney', Truckrobbery.Money)
            pickingup = false
