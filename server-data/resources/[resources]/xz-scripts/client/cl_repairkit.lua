...
RegisterNetEvent('Repairkit:start')
AddEventHandler('Repairkit:start', function(advanced)
    ...
    if targetVehicle ~= 0 and not reapiring then
        ...
        if dist < 1.5 then
            if (advanced and math.random(1,2) or math.random(1,4)) == 2 then
                TriggerServerEvent("Scripts:RemoveItem",advanced and "advancedrepairkit" or "repairkit", 1)
            end

            TriggerEvent("Scripts:repairanim",targetVehicle)
            ...
            local finished = exports["xz-taskbarskill"]:taskBar(15000,math.random(10,20))
            ...
            if advanced then
                ...
                local finished = exports["xz-taskbarskill"]:taskBar(timer,math.random(5,15))
                if finished ~= 100 then
                    fixingvehicle = false
                    reapiring = false
                    ClearPedTasks(playerped)
...
RegisterNetEvent('Scripts:repairanim')
AddEventHandler('Scripts:repairanim', function(veh)
