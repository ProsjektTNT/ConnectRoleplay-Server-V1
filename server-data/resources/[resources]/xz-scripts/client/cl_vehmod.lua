...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        local finished = exports["xz-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
...
CreateThread(function()
    local vehStatus = false
    while true do
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local inVeh = DoesEntityExist(veh)
        if inVeh and not vehStatus then
            vehStatus = true
            TriggerEvent("vehicle:entered", veh)
        elseif not inVeh and vehStatus then
            vehStatus = false
            TriggerEvent("vehicle:left")
        end
