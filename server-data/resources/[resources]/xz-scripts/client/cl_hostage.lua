...
RegisterNetEvent('action:TakeHostage', function()
    takeHostage()
end)
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(PlayerPedId(), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(PlayerPedId(), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					XZCore.Functions.Notify("You have no ammunition.", "error")
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			XZCore.Functions.Notify("You can\'t take someone hostage with this weapon", "error")
			return
		end
		...
		if not holdingHostageInProgress then		
			...
			holdingHostage = true 
			TriggerServerEvent('animation:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, false)
		end
	else
...
RegisterNetEvent('animation:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach, emote)
...
RegisterNetEvent('animation:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = PlayerPedId()
	ClearPedSecondaryTask(PlayerPedId())
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('animation:cl_stop', function()
	holdingHostageInProgress = false
	beingHeldHostage = false 
	...
	while true do 
		if holdingHostage then
			if IsEntityDead(PlayerPedId()) then	
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("animation:stop",target)
				Wait(100)
				releaseHostage()
			end 
...
function releaseHostage()
	...
	if closestPlayer ~= 0 then
		TriggerServerEvent('animation:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, true)
	end
end 
