    ...
    while true do
        ...
        if IsControlJustReleased(0, 47) then 
            if not harnessOn then
                ...
                XZCore.Functions.Progressbar("harness_equip", "Taking off race harness...", 5000, false, true, {
...
local handbrake = 0
RegisterNetEvent('resethandbrake', function()
...
RegisterNetEvent('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            XZCore.Functions.Progressbar("harness_equip", "Putting on race harness...", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        XZCore.Functions.Notify('You\'re not in a car.', 'error', 3500)
    end
end)
