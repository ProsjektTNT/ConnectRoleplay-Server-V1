...
local carspawns = {
	[1] =  { ['coords'] = vector4(-500.5054, -611.8494, 24.70247, 89.702308), ['info'] = ' car 8' },
	[2] =  { ['coords'] = vector4(-494.0621, -605.3046, 24.702302, 266.05514), ['info'] = ' car 1' },
	[3] =  { ['coords'] = vector4(-477.7127, -624.577, 24.703399, 180.17251), ['info'] = ' car 2' },
	[4] =  { ['coords'] = vector4(-475.1159, -611.8567, 24.702049, 269.7528), ['info'] = ' car 3' },
	[5] =  { ['coords'] = vector4(-495.2731, -625.0568, 24.703281, 179.1164), ['info'] = ' car 4' },
	[6] =  { ['coords'] = vector4(-514.2058, -599.652, 24.702228, 270.30624), ['info'] = ' car 5' },
	[7] =  { ['coords'] = vector4(-516.8527, -623.639, 24.702762, 234.07461), ['info'] = ' car 6' },
	[8] =  { ['coords'] = vector4(-463.8773, -624.3208, 24.702577, 359.76947), ['info'] = ' car 7' },
}

local pillWorker = { ['coords'] = vector4(-520.5541, -615.1972, 26.529947, 90.257888), ['info'] = ' store ' }
...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			XZCore.Functions.Notify('The delivery was on point, your GPS will be updated with the next drop off', 'success')
		else
			XZCore.Functions.Notify('The drop-off failed', 'error')
		end
...
RegisterNetEvent("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			XZCore.Functions.Notify('You are close to the drop-off point')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			XZCore.Functions.Notify('You took too long', 'error')
		end
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			DrawText3Ds(pillWorker['coords']["x"],pillWorker['coords']["y"],pillWorker['coords']["z"], "[E] $2,500 - Delivery Job (Payment Cash + Oxy)") 
	...
	while true do
		...
		if OxyRun then
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				XZCore.Functions.Notify("The dealer isn't giving you anymore locations due to the state of his car", "error")
			else
				if tasking then
					Citizen.Wait(30000)
				else
...
RegisterNetEvent("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true
	XZCore.Functions.Notify('A car has been left outside for you. Your pager will be updated with locations soon..', 'success')
	CreateOxyVehicle()
end)
