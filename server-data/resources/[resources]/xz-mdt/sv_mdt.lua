...
RegisterServerEvent("mdt:hotKeyOpen", function()
	local usource = source
    local xPlayer = XZCore.Functions.GetPlayer(usource)
...
RegisterServerEvent("mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
...
RegisterServerEvent("mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:fetch("SELECT * FROM `players` WHERE `charinfo` LIKE ?", {string.lower('%'..query..'%')}, function(result) -- % wildcard, needed to search for all alike results
		...
		for index, data in ipairs(result) do
			if data.charinfo then
				...
				local core = XZCore.Functions.GetPlayerByCitizenId(data.citizenid)
    ...
    exports.oxmysql:fetch('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {offender.id}, function(result)
        ...
        exports.oxmysql:fetch('SELECT * FROM `user_convictions` WHERE `char_id` = ?', {offender.id}, function(convictions)
            ...
            exports.oxmysql:fetch('SELECT * FROM `mdt_warrants` WHERE `char_id` = ?', {offender.id}, function(warrants)
				...
				exports.oxmysql:fetch('SELECT * FROM `xzvehicles` WHERE `citizenid` = ?', {}, function(vehicles)
					for i = 1, #vehicles do
        ...
        exports.oxmysql:fetch('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {offender.id}, function(result)
            ...
            exports.oxmysql:fetch('SELECT * FROM `user_convictions` WHERE `char_id` = ?', {offender.id}, function(convictions) 
                ...
                exports.oxmysql:fetch('SELECT * FROM `mdt_warrants` WHERE `char_id` = ?', {offender.id}, function(warrants)
                    ...
                    exports.oxmysql:fetch('SELECT * FROM `xzvehicles` WHERE `citizenid` = ?', {offender.citizenid}, function(vehicles)
                        for i = 1, #vehicles do
                            vehicles[i].model = vehicles[i].vehicle
                            if vehicles[i].mods then
                                local vehmods = json.decode(vehicles[i].mods)
...
RegisterServerEvent("mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	exports['oxmysql']:fetch("SELECT * FROM `xzvehicles` WHERE `plate` LIKE @query", {
...
RegisterServerEvent("mdt:performVehicleSearchInFront", function(query)
	local usource = source
	local xPlayer = XZCore.Functions.GetPlayer(usource)
    if xPlayer.PlayerData.job.name == 'police' then
    	exports.oxmysql:fetch("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		exports.oxmysql:fetch("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			exports.oxmysql:fetch("SELECT * FROM `xzvehicles` WHERE `plate` = ?", {query}, function(result)
					local officer = GetCharacterName(usource)
...
function GetLicenses(identifier, cb)
	local player = XZCore.Functions.GetPlayerByCitizenId(identifier)
	if player ~= nil then
