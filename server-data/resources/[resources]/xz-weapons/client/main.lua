            ...
            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        if XZCore.Shared.Weapons[weapon]["name"] == "weapon_snowball" then
                            TriggerServerEvent('XZCore:Server:RemoveItem', "snowball", 1)
                        else
                            if ammo > 0 then
                        ...
                        if weapon ~= -1569615261 then
                            TriggerEvent('inventory:client:CheckWeapon', XZCore.Shared.Weapons[weapon]["name"])
                            XZCore.Functions.Notify("This weapon is broken and can not be used..", "error")
                            MultiplierAmount = 0
                        end
                    end
                end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedShooting(ped) then
            if ammo - 1 < 1 and XZCore.Shared.Weapons[weapon]["name"] ~= "weapon_taser" then
                SetAmmoInClip(ped, GetHashKey(XZCore.Shared.Weapons[weapon]["name"]), 1)
            end
        end
...
RegisterNetEvent('xz-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = GetPlayerPed(-1)
    if weapon ~= "current" then
        local weapon = weapon:upper()
        SetPedAmmo(ped, GetHashKey(weapon), ammo)
        XZCore.Functions.Notify('+'..ammo..' Ammo for the '..XZCore.Shared.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            TriggerServerEvent("weapons:server:AddWeaponAmmo", CurrentWeaponData, ammo)
            XZCore.Functions.Notify('+'..ammo..' Ammo for the '..XZCore.Shared.Weapons[weapon]["label"], 'success')
        else
            XZCore.Functions.Notify('You dont have a weapon..', 'error')
        end
    end
end)
...
RegisterNetEvent('weapon:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData ~= nil then
        if XZCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and XZCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            local total = GetAmmoInPedWeapon(ped, weapon)
            local found, maxAmmo = GetMaxAmmo(ped, weapon)

            if total < maxAmmo then
                XZCore.Functions.Progressbar("taking_bullets", "Loading bullets..", math.random(4000, 6000), false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    if XZCore.Shared.Weapons[weapon] ~= nil then
                        AddAmmoToPed(ped,weapon,amount)
                        TaskReloadWeapon(ped)
                        TriggerServerEvent("weapons:server:AddWeaponAmmo", CurrentWeaponData, total + amount)
                        TriggerServerEvent('XZCore:Server:RemoveItem', itemData.name, 1, itemData.slot)
                        TriggerEvent('inventory:client:ItemBox', XZCore.Shared.Items[itemData.name], "remove")
                        TriggerEvent('XZCore:Notify', 'Reloaded', "success")
                    end
                end, function()
                    XZCore.Functions.Notify("Canceled", "error")
                end)
            else
                XZCore.Functions.Notify("Max Ammo Capacity", "error")
            end
        else
            XZCore.Functions.Notify("You Have No Weapon", "error")
        end
    else
        XZCore.Functions.Notify("You Have No Weapon.", "error")
    end
...
RegisterNetEvent('XZCore:Client:OnPlayerLoaded', function()
    ...
    XZCore.Functions.TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
                ...
                if distance < 10 then
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            ...
                            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                                if not data.RepairingData.Ready then
                                    local WeaponData = XZCore.Shared.Weapons[GetHashKey(CurrentWeaponData.name)]
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        XZCore.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
...
RegisterNetEvent("weapons:client:EquipAttachment", function(ItemData, attachment)
    local ped = PlayerPedId()
    local weapon = GetSelectedPedWeapon(ped)
    local WeaponData = XZCore.Shared.Weapons[weapon]
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if WeaponAttachments[WeaponData.name] ~= nil then
            if WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                XZCore.Functions.Notify("This weapon does not support this attachment..", "error")
            end
        end
    else
        XZCore.Functions.Notify("You dont have a weapon in ur hand..", "error")
    end
end)
