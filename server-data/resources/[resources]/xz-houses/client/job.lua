...
CreateThread(function()
    Wait(500)
    TriggerServerEvent('xz-houses:server:doorState')
    while true do
        ...
        local distance = {}
        local whitelisted = (XZCore and XZCore.Functions.GetPlayerData().job and XZCore.Functions.GetPlayerData().job.name == 'realestate')
        local isboss = (XZCore and XZCore.Functions.GetPlayerData().job and XZCore.Functions.GetPlayerData().job.isboss)
        ...
        if (distance['enter'] < 5) then
            if (whitelisted) then
                ...
                if (IsControlJustPressed(0, 74)) then 
                    TriggerServerEvent('xz-houses:server:doorState', not DoorLocked)
                end
            else
...
RegisterNetEvent('xz-houses:client:doorState', function(bool)
    DoorLocked = bool
end)
