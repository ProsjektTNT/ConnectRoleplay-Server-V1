	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				XZCore.Functions.Notify("You have gone out of range!")
			end
		end
	end
end)

RegisterNetEvent("xz-houses:client:decorate")
AddEventHandler("xz-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			XZCore.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNetEvent("xz-houses:server:sethousedecorations")
AddEventHandler("xz-houses:server:sethousedecorations", function(house, decorations)
...
function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('xz-anticheat:client:ToggleDecorate', true)
end
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('xz-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		XZCore.Functions.TriggerCallback('xz-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("xz-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		XZCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
