...
RegisterNetEvent('xz-bossmenu:client:openMenu')
AddEventHandler('xz-bossmenu:client:openMenu', function(employees, jobdata)
...
RegisterNetEvent('xz-bossmenu:client:refreshPage')
AddEventHandler('xz-bossmenu:client:refreshPage', function(data, list)
...
RegisterNetEvent('xz-bossmenu:client:refreshSociety')
AddEventHandler('xz-bossmenu:client:refreshSociety', function(job, data)
    if XZCore and XZCore.Functions.GetPlayerData().job.name == job then
        SendNUIMessage({
            open = true,
...
RegisterNUICallback('openStash', function(data)
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "boss_" .. XZCore.Functions.GetPlayerData().job.name, {
        maxweight = 4000000,
        slots = 500,
    })

    TriggerEvent("inventory:client:SetCurrentStash", "boss_" .. XZCore.Functions.GetPlayerData().job.name)
end)

RegisterNUICallback('giveJob', function(data)
    TriggerServerEvent('xz-bossmenu:server:giveJob', data)
end)
...
RegisterNUICallback('openRecruit', function(data)
    CreateThread(function()
        ...
        for k,v in pairs(XZCore.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
            if v and v ~= PlayerId() then
                table.insert(players, GetPlayerServerId(v))
            end
        end

        TriggerServerEvent("xz-bossmenu:server:updateNearbys", players)
    end)
end)

RegisterNUICallback('changeGrade', function(data)
    TriggerServerEvent('xz-bossmenu:server:updateGrade', data)
end)

RegisterNUICallback('fireEmployee', function(data)
    TriggerServerEvent('xz-bossmenu:server:fireEmployee', data)
end)
...
RegisterNUICallback('withdraw', function(data)
    ...
    TriggerServerEvent("xz-bossmenu:server:withdrawMoney", amount)
end)

RegisterNUICallback('deposit', function(data)
    local amount = tonumber(data.amount)
    TriggerServerEvent("xz-bossmenu:server:depositMoney", amount)
end)
