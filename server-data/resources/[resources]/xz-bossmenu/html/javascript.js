...
addEventListener("message", function(event){
    ...
    if(item.open == true) {
        ...
        $.post('http://xz-bossmenu/closeNUI');
    }
});
...
$(document).on('click', '.fireemployee', function(e) {
    ...
    $.post('http://xz-bossmenu/fireEmployee', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.grade-box', function(e) {
    ...
    $.post('http://xz-bossmenu/changeGrade', JSON.stringify({
        grade: gradeid,
...
$(document).on('click', '#option-recruit', function(e) {
    ...
    $.post('http://xz-bossmenu/openRecruit', JSON.stringify({}));
});

$(document).on('click', '#option-bosstash', function(e) {

    $.post('http://xz-bossmenu/openStash', JSON.stringify({}));
});
...
$(document).on('click', '.givejob', function(e) {
    ...
    $.post('http://xz-bossmenu/giveJob', JSON.stringify({
        source: playerid,
    }));
});

$(document).on('click', '.withdraw-down', function(e) {
    var newAmount = currentWithdrawAmount - 100;
...
$(document).on('click', '.deposit-down', function(e) {
    var newAmount = currentDepositAmount - 100;
...
$(document).on('click', '.deposit', function(e) {
    $.post('http://xz-bossmenu/deposit', JSON.stringify({
        amount: currentDepositAmount,
    }));
});

$(document).on('click', '.withdraw', function(e) {
    $.post('http://xz-bossmenu/withdraw', JSON.stringify({
