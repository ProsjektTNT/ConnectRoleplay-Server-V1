...
AddEventHandler("platecheck:frontradar", function()
    TriggerServerEvent('platecheck:checkLicensePlate',radarInfo.frontPlate, radarInfo.frontModel)
end)

RegisterNetEvent("platecheck:rearradar")
AddEventHandler("platecheck:rearradar", function()
    TriggerServerEvent('platecheck:checkLicensePlate',radarInfo.rearPlate, radarInfo.rearModel)
end)
    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        local vehicle = GetVehiclePedIsIn( ped, false )
            ...
            if ( radarEnabled ) then 
                XZCore.Functions.Notify( "Radar enabled." )
            else 
                XZCore.Functions.Notify( "Radar disabled.",'error' )
            end 
...
AddEventHandler( 'wk:toggleRadar', function()
    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        XZCore.Functions.Notify( "Not in vehicle.",'error' )
    end 
end )
...
function ToggleSpeedType()
    ...
    XZCore.Functions.Notify( "Speed type MPH." )
end 

function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        radarInfo.lockBeep = false 
        XZCore.Functions.Notify( "Radar beep disabled.",'error' )
    else 
                ...
                if ( radarInfo.fwdXmit ) then  
                    ...
                    if ( DoesEntityExist( fwdVeh ) and IsEntityAVehicle( fwdVeh ) ) then 
                        ...
                        if ( fwdVehSpeed > radarInfo.fastLimit and not radarInfo.fwdFastLocked ) and not lockplates then 
                            ...
                            radarInfo.frontModel = GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(fwdVeh)))
                ...
                if ( radarInfo.bwdXmit ) then 
                    ...
                    if ( DoesEntityExist( bwdVeh ) and IsEntityAVehicle( bwdVeh ) ) then
                        ...
                        if ( bwdVehSpeed > radarInfo.fastLimit and not radarInfo.bwdFastLocked ) and not lockplates then 
                            ...
                            radarInfo.rearModel = GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(bwdVeh)))
...
Citizen.CreateThread( function()
    ...
    while true do 
        ...
        if GetPedInVehicleSeat( veh, -1 ) == ped then
            ...
            if ( IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 243 ) and IsPedSittingInAnyVehicle( ped ) and currentJob == 'police' ) then 

                lockplates = not lockplates
                if lockplates then
                    XZCore.Functions.Notify('Radar Locked', 'error')
                else
                    XZCore.Functions.Notify('Radar Unlocked')
                end
            end 

            -- LCtrl is not being pressed and M has just been pressed 
            if ( not IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 243 ) and IsPedSittingInAnyVehicle( ped ) and currentJob == 'police' ) then 
                ResetFrontFast()
                ResetRearFast()
                XZCore.Functions.Notify('Lockfast Reset')
            end 

            if ( IsDisabledControlJustPressed( 1, 166 ) and currentJob == 'police' and IsPedSittingInAnyVehicle( ped ) ) then 
   ...
   Citizen.Wait(1)
       ...
       if currentJob == 'police' then
           ...
           if isInVeh then
               ...
               if IsControlJustReleased(0,172) or IsDisabledControlJustReleased(0,172) and radarEnabled then
                   TriggerEvent("platecheck:frontradar")
                   Citizen.Wait(400)
               end

               if IsControlJustReleased(0,173) and radarEnabled then
                   TriggerEvent("platecheck:rearradar")
                   Citizen.Wait(400)
               end

               if IsControlJustReleased(0,174) and radarEnabled then
