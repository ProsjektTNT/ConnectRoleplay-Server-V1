...
RegisterNetEvent('xz-fitbit:use')
AddEventHandler('xz-fitbit:use', function()
  openWatch(true)
end)
...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    TriggerServerEvent('xz-fitbit:server:setValue', 'food', foodValue)

    XZCore.Functions.Notify('Fitbit: Food warning set to '..foodValue..'%')
end)

RegisterNUICallback('setThirstWarning', function(data)
    local thirstValue = tonumber(data.value)

    TriggerServerEvent('xz-fitbit:server:setValue', 'thirst', thirstValue)

    XZCore.Functions.Notify('Fitbit: Thirst warning set to '..thirstValue..'%')
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            XZCore.Functions.TriggerCallback('xz-fitbit:server:HasFitbit', function(hasItem)
                if hasItem then
