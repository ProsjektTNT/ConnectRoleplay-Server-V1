...
local function checkExistenceClothes(citizenid, cb)
    exports.oxmysql:fetch("SELECT citizenid FROM character_current WHERE citizenid = '" .. citizenid .. "'", {}, function(result)
...
local function checkExistenceFace(citizenid, cb)
    exports.oxmysql:fetch("SELECT citizenid FROM character_face WHERE citizenid = '" .. citizenid .. "' LIMIT 1;", {["citizenid"] = citizenid}, function(result)
        local exists = result and result[1] and true or false
        cb(exists)
    end)
end
...
AddEventHandler("raid_clothes:insert_character_current",function(data)
    ...
    local src = source
    local user = XZCore.Functions.GetPlayer(src)
    ...
    checkExistenceClothes(characterId, function(exists)
        ...
        if not exists then
            ...
            local vals = string.format("'%s', '%s', '%s', '%s', '%s', '%s'", characterId, values["@model"], values["@drawables"], values["@props"], values["@drawtextures"], values["@proptextures"])

            exports.oxmysql:insert("INSERT INTO character_current ("..cols..") VALUES ("..vals..")", {}, function()
            end)
            return
        end
        ...
        print("19")
        exports.oxmysql:execute("UPDATE character_current SET "..set.." WHERE citizenid = '" .. characterId .. "'", {})
    end)
end)
...
AddEventHandler("raid_clothes:insert_character_face",function(data)
    ...
    checkExistenceFace(characterId, function(exists)
        ...
        if not exists then
            ...
            exports.oxmysql:insert("INSERT INTO character_face ("..cols..") VALUES ("..vals..")", {}, function()
            end)
            return
        end
...
AddEventHandler("raid_clothes:get_character_face",function(pSrc)
    ...
    exports.oxmysql:fetch("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM character_face cf INNER JOIN character_current cc on cc.citizenid = cf.citizenid WHERE cf.citizenid = '" .. characterId .. "'", {}, function(result)
...
AddEventHandler("raid_clothes:get_character_current",function(pSrc)
    ...
    exports.oxmysql:fetch("SELECT * FROM character_current WHERE citizenid = '" .. characterId .. "'", {}, function(result)
        local temp_data = {
...
AddEventHandler("raid_clothes:retrieve_tats", function(pSrc)
    ...
    print("SELECT * FROM playersTattoos WHERE citizenid = '" .. char .. "'")
	exports.oxmysql:fetch("SELECT * FROM playersTattoos WHERE citizenid = '" .. char .. "'", {}, function(result)
        print("after fetching at ima shalo")
        if(#result == 1) then
            print("checking if ima shalo zona")
			TriggerClientEvent("raid_clothes:settattoos", src, json.decode(result[1].tattoos))
		else
            print("bb gever")
			local tattooValue = "{}"
			exports.oxmysql:insert("INSERT INTO playersTattoos (citizenid, tattoos) VALUES ('" .. char .. "', '" .. tattooValue .. "')", {})
...
AddEventHandler("raid_clothes:get_outfit",function(slot)
    ...
    exports.oxmysql:fetch("SELECT * FROM character_outfits WHERE citizenid = '" .. characterId .. "' and slot = '" .. slot .. "'", {}, function(result)
...
AddEventHandler("raid_clothes:set_outfit",function(slot, name, data)
    ...
    exports.oxmysql:fetch("SELECT slot FROM character_outfits WHERE citizenid = '" .. characterId .. "' and slot = '" .. slot .. "'", {}, function(result)
        if result and result[1] then
            local values = {
                ["@citizenid"] = characterId,
                ["@slot"] = slot,
            ...
            print("20")
            exports.oxmysql:execute("UPDATE character_outfits SET "..set.." WHERE citizenid = '" .. characterId .. "' and slot = '" .. slot .. "'", {})
        else
            ...
            exports.oxmysql:insert("INSERT INTO character_outfits ("..cols..") VALUES ("..vals..")", {}, function()
...
AddEventHandler("raid_clothes:list_outfits",function()
    ...
    exports.oxmysql:fetch("SELECT slot, name FROM character_outfits WHERE citizenid = '" .. citizenid .. "'", {}, function(skincheck)
...
AddEventHandler("clothing:checkIfNew", function()
    ...
    exports.oxmysql:fetch("SELECT model FROM character_current WHERE citizenid = '" .. citizenid .. "'", {}, function(result)
...
AddEventHandler("clothing:checkMoney", function(menu,askingPrice)
    local src = source
    local target = XZCore.Functions.GetPlayer(src)
...
RegisterServerEvent("xz-clothing:stealOutfit")
AddEventHandler("xz-clothing:stealOutfit", function(target)
    TriggerClientEvent("xz-clothing:stealOutfit", target, source)
end)

RegisterServerEvent("xz-clothing:letSteal")
AddEventHandler("xz-clothing:letSteal", function(cheater, outfit)
    TriggerClientEvent("xz-clothing:letSteal", cheater, outfit)
end)