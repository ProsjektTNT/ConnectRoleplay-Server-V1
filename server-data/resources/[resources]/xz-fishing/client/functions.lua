function TryToFish()
    XZCore.Functions.TriggerCallback('xz-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return XZCore.Functions.Notify("You can't be swimming and fishing at the same time.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return XZCore.Functions.Notify("You need to exit your vehicle to start fishing.", "error") end 
        if count ~= nil then
            if count == 0 then
                XZCore.Functions.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    XZCore.Functions.Notify("You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        XZCore.Functions.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            XZCore.Functions.Notify("You need to cast the bait.", "primary")
    ...
    XZCore.Functions.Notify("Waiting for a fish to bite...", "primary", "10000")
    TriggerServerEvent('XZCore:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("inventory:client:ItemBox", XZCore.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        TriggerServerEvent("xz-fishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('xz-hud:Server:RelieveStress', 1)
    else
        XZCore.Functions.Notify("The fish got loose.", "error")
    end
...
function IsInWater()
    ...
    XZCore.Functions.Notify("Checking fishing location...", "primary", "3000")
...
function SellFish()
    XZCore.Functions.TriggerCallback('xz-fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("xz-fishing:sellFish", function(sold, fishesSold) end)
    end)
end
...
function HasFishingBait()
    local rtval = false
    XZCore.Functions.TriggerCallback('XZCore:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
