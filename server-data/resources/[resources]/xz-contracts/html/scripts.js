$(document).ready(function(){
  ...
  $(".btnPrev").click(function(){
      $.post('http://xz-contracts/previousID', JSON.stringify({}));
  });

  $(".btnNext").click(function(){
      $.post('http://xz-contracts/nextID', JSON.stringify({}));
  });

  $(".btnGive").click(function(){
      $.post('http://xz-contracts/giveID', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });

  $(".btnAccept").click(function(){
    $.post('http://xz-contracts/accept', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });

  $(".btnDeny").click(function(){
    $.post('http://xz-contracts/deny', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });
  ...
  document.onkeyup = function (data) {
    if ( data.which == 27 ) {
      $.post('http://xz-contracts/close', JSON.stringify({}));
    }
    if ( data.which == 37 ) {
      $.post('http://xz-contracts/previousID', JSON.stringify({}));
    }
