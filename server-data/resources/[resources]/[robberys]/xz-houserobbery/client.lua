...
AddEventHandler("XZCore:Client:OnJobUpdate", function(jobInfo)
 ...
 RegisterCommand('breach', function(source, args)  
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'police' then 
   ...
   if not found then  
    TriggerEvent('XZCore:Notify', "No house", "error") 
   end  
  else  
   TriggerEvent('XZCore:Notify', "You are not police", "error")  
  end 
 end)
...
AddEventHandler('lockpicks:UseLockpick', function(advanced)
 ...
 if isNight() and not isRobbing then
  XZCore.Functions.TriggerCallback("houserob:checkcops", function(copsNum)
    if copsNum >= Config.MinCops then
      ...
      for id,v in pairs(Config.RobbableHouses) do
        if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
          ...
          XZCore.Functions.Progressbar("lockpicking_property", "Lockpicking Property", 12000, false, true, {}, {}, {}, {}, function()
            isLockpicking = false
            pedSpawned = false
      ...
      TriggerEvent('XZCore:Notify', 'Not enough Police.', "error")
    end
  end)
end
  
  if not isNight() and not isRobbing then
    TriggerEvent('XZCore:Notify', 'Its too bright out.', "error")
  end
end)
   ...
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(PlayerPedId())) < 1.4) and not myRobbableItems[i]['isSearched'] then
     ...
     if IsControlJustReleased(1, 74) then
      ...
      XZCore.Functions.Progressbar("search_"..myRobbableItems[i]['name'], "Searching "..myRobbableItems[i]['name'], 20000, false, true, {}, {}, {}, {}, function()
