...
RegisterServerEvent('xz-storerobbery:server:takeMoney')
AddEventHandler('xz-storerobbery:server:takeMoney', function(register, isDone)
    local src   = source
    ...
    if isDone then
        if math.random(1, 100) <= 17 then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, XZCore.Shared.Items["stickynote"], "add")
        end
    end
end)

RegisterServerEvent('xz-storerobbery:server:setRegisterStatus')
AddEventHandler('xz-storerobbery:server:setRegisterStatus', function(register)
	local Player = XZCore.Functions.GetPlayer(src)
    TriggerClientEvent('xz-storerobbery:client:setRegisterStatus', -1, register, true)
    Config.Registers[register].robbed   = true
    Config.Registers[register].time     = Config.resetTime
	dclog(Player, 'Store has been robbed.')
end)

RegisterServerEvent('xz-storerobbery:server:setSafeStatus')
AddEventHandler('xz-storerobbery:server:setSafeStatus', function(safe)
    TriggerClientEvent('xz-storerobbery:client:setSafeStatus', -1, safe, true)
    Config.Safes[safe].robbed = true

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        TriggerClientEvent('xz-storerobbery:client:setSafeStatus', -1, safe, false)
        Config.Safes[safe].robbed = false
    end)
end)

RegisterServerEvent('xz-storerobbery:server:SafeReward')
AddEventHandler('xz-storerobbery:server:SafeReward', function(safe)
    local src = source
    local Player = XZCore.Functions.GetPlayer(src)
...
RegisterServerEvent('xz-storerobbery:server:callCops')
AddEventHandler('xz-storerobbery:server:callCops', function(type, safe)
    local cameraId = 4
    if type == "safe" then
    ...
    while true do
        for k, v in pairs(Config.Registers) do
            if Config.Registers[k].time > 0 and (Config.Registers[k].time - Config.tickInterval) >= 0 then
                ...
                TriggerClientEvent('xz-storerobbery:client:setRegisterStatus', -1, k, false)
            end
        end
...
XZCore.Functions.CreateCallback('xz-storerobbery:server:isCombinationRight', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

XZCore.Functions.CreateCallback('xz-storerobbery:server:getPadlockCombination', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

XZCore.Functions.CreateCallback('xz-storerobbery:server:getRegisterStatus', function(source, cb)
    cb(Config.Registers)
end)

XZCore.Functions.CreateCallback('xz-storerobbery:server:getSafeStatus', function(source, cb)
    cb(Config.Safes)
end)
