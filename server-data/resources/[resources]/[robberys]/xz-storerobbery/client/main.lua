...
Citizen.CreateThread(function()
    Wait(1000)
    if XZCore.Functions.GetPlayerData().job ~= nil and next(XZCore.Functions.GetPlayerData().job) then
                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe].x, Config.Safes[safe].y, Config.Safes[safe].z, '[E] - Insert Combination')
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if math.random(100) <= 50 then
                                        TriggerServerEvent('xz-hud:Server:GainStress', math.random(1, 3))
                                    end
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        XZCore.Functions.TriggerCallback('xz-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if copsCalled == false then
                                        ...
                                        TriggerServerEvent("xz-storerobbery:server:callCops", "safe", currentSafe)
                                        copsCalled = true
                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if math.random(1, 10) < 5 then
                        TriggerServerEvent("xz-storerobbery:server:callCops", "cashier", currentRegister)   
                    ...
                    XZCore.Functions.TriggerCallback('XZCore:HasItem', function(result)
                        if result then
                            ...
                            XZCore.Functions.Notify("You are missing a screwdriver", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    XZCore.Functions.TriggerCallback('xz-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    XZCore.Functions.TriggerCallback('xz-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('xz-storerobbery:server:setRegisterStatus', currentRegister)
        local lockpickTime = 30000
        LockpickDoorAnim(lockpickTime)
        XZCore.Functions.Progressbar("search_register", "Emptying Register", lockpickTime, false, true, {
            ...
            TriggerServerEvent('xz-storerobbery:server:takeMoney', currentRegister, true)            
            currentRegister = 0
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(PlayerPedId())
...
function LockpickDoorAnim(time)
    ...
    Citizen.CreateThread(function()
        while openingDoor do
            ...
            Citizen.Wait(2000)
            time = time - 2
            TriggerServerEvent('xz-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("xz-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("xz-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function()
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            TriggerServerEvent("XZCore:Server:RemoveItem", "advancedlockpick", 1)
            TriggerEvent('inventory:client:ItemBox', XZCore.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("XZCore:Server:RemoveItem", "lockpick", 1)
            TriggerEvent('inventory:client:ItemBox', XZCore.Shared.Items["lockpick"], "remove")
        end
    end
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        XZCore.Functions.Notify("The lockpick bent out of shape.")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    XZCore.Functions.TriggerCallback('xz-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('xz-storerobbery:client:setRegisterStatus')
AddEventHandler('xz-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('xz-storerobbery:client:setSafeStatus')
AddEventHandler('xz-storerobbery:client:setSafeStatus', function(safe, bool)
