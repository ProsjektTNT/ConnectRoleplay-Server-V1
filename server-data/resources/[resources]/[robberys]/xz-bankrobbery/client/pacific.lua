...
Citizen.CreateThread(function()
    ...
    local requiredItems = {
		[1] = {name = XZCore.Shared.Items["electronickit"]["name"], image = XZCore.Shared.Items["electronickit"]["image"]},
		[2] = {name = XZCore.Shared.Items["security_card_02"]["name"], image = XZCore.Shared.Items["security_card_02"]["image"]},
    }
    while true do
    ...
    if dist < 1.5 then
        XZCore.Functions.TriggerCallback('xz-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                                    ...
                                    XZCore.Functions.Progressbar("hack_gate", "Connecting Electronic Kit", 7500, false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("xz-bankrobbery:server:callCops", "pacific", 0)
                ...
                XZCore.Functions.Notify("The security lock is active, the door cannot be opened atm", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('xz-bankrobbery:UseBankcardB')
AddEventHandler('xz-bankrobbery:UseBankcardB', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
        ...
        XZCore.Functions.TriggerCallback('xz-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        XZCore.Functions.Progressbar("security_pass", "Adjust to validate", 7500, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('xz-doorlock:server:updateState', 67, false)
                            TriggerServerEvent("XZCore:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('xz-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
