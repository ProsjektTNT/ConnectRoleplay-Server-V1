...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = XZCore.Shared.Items["security_card_01"]["name"], image = XZCore.Shared.Items["security_card_01"]["image"]},
		[2] = {name = XZCore.Shared.Items["electronickit"]["name"], image = XZCore.Shared.Items["electronickit"]["image"]},
    }
...
RegisterNetEvent('xz-bankrobbery:UseBankcardA')
AddEventHandler('xz-bankrobbery:UseBankcardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["coords"]["x"], Config.BigBanks["paleto"]["coords"]["y"],Config.BigBanks["paleto"]["coords"]["z"])
                ...
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        ...
                        XZCore.Functions.Progressbar("security_pass", "Adjust to validate", 7500, false, true, {
                            ...
                            TriggerServerEvent('xz-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("XZCore:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('xz-doorlock:server:updateState', 78, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("xz-bankrobbery:server:callCops", "paleto", 0)
