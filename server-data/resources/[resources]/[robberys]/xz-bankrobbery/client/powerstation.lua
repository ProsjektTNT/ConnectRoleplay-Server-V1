...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    requiredItems = {
        [1] = {name = XZCore.Shared.Items["thermite"]["name"], image = XZCore.Shared.Items["thermite"]["image"]},
    }
    while true do
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    XZCore.Functions.Notify("It seems that the fuses have blown", "error")
                end
            else
...
RegisterNetEvent('xz-bankrobbery:client:SetStationStatus')
AddEventHandler('xz-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("XZCore:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', XZCore.Shared.Items["thermite"], "remove")
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(PlayerPedId())
    while time > 0 do 
        XZCore.Functions.Notify("Burning over " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    ...
    if currentStation ~= 0 then
        XZCore.Functions.Notify("The fuses are broken", "success")
        TriggerServerEvent("xz-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        XZCore.Functions.Notify("The door is burned open", "success")
        TriggerServerEvent('xz-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
