...
RegisterServerEvent("Drugs:reward", function(items)
...
RegisterServerEvent("Drugs:syncMissionData", function(data)
    TriggerClientEvent("Drugs:syncMissionData",-1,data)
end)

XZCore.Functions.CreateUseableItem('methburn', function(source, item)
    if not CheckedStarted(GetPlayerIdentifier(source)) then
        ...
        TriggerEvent("xz-logs:server:sendLog", {User = source}, "usb", "Meth USB Used", "", {}, "green", "xz-usb")
        TriggerClientEvent("Drugs:UsableItem",source)
        Citizen.Wait(8000)
        TriggerClientEvent("Drugs:HackingMiniGame",source)
        XZCore.Functions.CreateCallback("Drugs:StartMissionNow",function(source,cb)
        ...
        TriggerClientEvent("XZCore:Notify",source,string.format("You can hack the network again in: %s minutes",GetTimeForMission(GetPlayerIdentifier(source))), 'error')
    end
end)

XZCore.Functions.CreateUseableItem('cokeburn', function(source, item)
    if not CheckedStarted(GetPlayerIdentifier(source)) then
        local xPlayer = XZCore.Functions.GetPlayer(source)
        xPlayer.Functions.RemoveItem('cokeburn', 1)
        TriggerEvent("Drugs:startHackTimer",source)
        TriggerEvent("xz-logs:server:sendLog", {User = source}, "usb", "Coke USB Used", "", {}, "green", "xz-usb")
        TriggerClientEvent("Drugs:UsableItem",source)
        Citizen.Wait(8000)
        TriggerClientEvent("Drugs:HackingMiniGame",source)
...
XZCore.Functions.CreateUseableItem('weedburn', function(source, item)
    if not CheckedStarted(GetPlayerIdentifier(source)) then
        local xPlayer = XZCore.Functions.GetPlayer(source)
        xPlayer.Functions.RemoveItem('weedburn', 1)
        TriggerEvent("Drugs:startHackTimer",source)
        TriggerEvent("xz-logs:server:sendLog", {User = source}, "usb", "Weed USB Used", "", {}, "green", "xz-usb")
        TriggerClientEvent("Drugs:UsableItem",source)
        Citizen.Wait(8000)
        TriggerClientEvent("Drugs:HackingMiniGame",source)
...
XZCore.Functions.CreateUseableItem('goldenburn', function(source, item)
    if not CheckedStarted(GetPlayerIdentifier(source)) then
        local xPlayer = XZCore.Functions.GetPlayer(source)
        xPlayer.Functions.RemoveItem('goldenburn', 1)
        TriggerEvent("Drugs:startHackTimer", source)
        local selected = string.gsub(Config.Items.items[math.random(1, 3)].name, "burn", "")
        TriggerEvent("xz-logs:server:sendLog", {User = source}, "usb", "Golden USB Used", " Random Type: `" .. selected .. "`", {}, "green", "xz-usb")
        TriggerClientEvent("Drugs:UsableItem", source)
        Citizen.Wait(8000)
        TriggerClientEvent("Drugs:HackingMiniGame",source)
...
-- COKE BRICK >> COKE (10G)
XZCore.Functions.CreateUseableItem('cokebrick', function(source, item)
    ...
    if not bags or not brick then
        if not bags then
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough bags", 'error')
        else
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough Cocaine Bricks", 'error')
        end
        return
    end
    ...
    if GetItem(source, "coke10g").count <= 40 or (not scale and GetItem(source, "coke10g").count <= 44) then
        if not CheckedStartedConvert(GetPlayerIdentifier(source)) then
            ...
            TriggerClientEvent("XZCore:Notify",source,string.format("You are already engaged in a process!",GetTimeForConvert(GetPlayerIdentifier(source))), 'error')
        end
    else
        TriggerClientEvent("XZCore:Notify",source,"You do not have enough empty space for more Cocaine (10G)", 'error')
    end
end)

-- METH BRICK >> METH (10G)
XZCore.Functions.CreateUseableItem('methbrick', function(source, item)
    ...
    if not bags or not brick then
        if not bags then
            ...
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough Meth Bricks", 'error')
        end
        return
    end
    ...
    if GetItem(source, "meth10g").count <= 40 or (not scale and GetItem(source, "meth10g").count <= 44) then
        if not CheckedStartedConvert(GetPlayerIdentifier(source)) then
            ...
            TriggerClientEvent("XZCore:Notify",source,string.format("You are already engaged in a process!",GetTimeForConvert(GetPlayerIdentifier(source))))
        end
    else
        TriggerClientEvent("XZCore:Notify",source,"You do not have enough empty space for more Meth (10G)", 'error')
    end
end)

-- COKE (10G) >> COKE (1G)
XZCore.Functions.CreateUseableItem('coke10g', function(source, item)
    ...
    if not bags or not coke then
        if not bags then
            ...
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough Cocaine (10G)", 'error')
        end
        return
    end
    ...
    if GetItem(source, "cokebaggy").count <= 40 or (not scale and GetItem(source, "cokebaggy").count <= 44) then
        ...
        TriggerClientEvent("XZCore:Notify",source,"You do not have enough empty space for more Cocaine (1G)", 'error')
    end
end)

-- METH (10G) >> METH (1G)
XZCore.Functions.CreateUseableItem('meth10g', function(source, item)
    ...
    if not bags or not meth then
        if not bags then
            ...
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough Meth (10G)", 'error')
        end
        return
    end
    ...
    if GetItem(source, "crack_baggy").count <= 40 or (not scale and GetItem(source, "crack_baggy").count <= 44) then
        ...
        TriggerClientEvent("XZCore:Notify",source,"You do not have enough empty space for more Meth (1G)", 'error')
    end
end)

-- WEED (4G) >> JOINT (2G)
XZCore.Functions.CreateUseableItem('weed4g', function(source, item)
    ...
    if not paper or not weed then
        if not paper then
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough rolling paper", 'error')
        else
            TriggerClientEvent("XZCore:Notify",source,"You do not have enough Weed (4G)", 'error')
        end
        return
    end
    ...
    if GetItem(source, "joint").count <= 48 then
        ...
        TriggerClientEvent("XZCore:Notify",source,"You do not have enough empty space for more Joint (2G)", 'error')
    end
end)
