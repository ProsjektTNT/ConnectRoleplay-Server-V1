...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    exports.oxmysql:execute('INSERT INTO apartments (name, type, label, citizenid) VALUES (@name, @type, @label, @citizenid)', {
        ['@name'] = apartmentId,
        ['@type'] = type,
        ['@label'] = label,
        ['@citizenid'] = Player.PlayerData.citizenid
    })
    TriggerClientEvent('XZCore:Notify', src, "You got a apartment ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    ...
    TriggerClientEvent('XZCore:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		exports.oxmysql:fetch("SELECT COUNT(*) as count FROM `apartments` WHERE `name` = ?", { tostring(type .. AparmentId) }, function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    exports.oxmysql:fetch("SELECT * FROM `apartments` WHERE `name` = ?", { apartmentId }, function(result)
...
function GetOwnedApartment(citizenid)
    exports.oxmysql:fetch("SELECT * FROM `apartments` WHERE `citizenid` = ?", { citizenid }, function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
XZCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
XZCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
XZCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
XZCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
        exports.oxmysql:execute('SELECT * FROM apartments WHERE citizenid=@citizenid', {['@citizenid'] = cid}, function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
        ...
        local src = source
        local Player = XZCore.Functions.GetPlayer(src)
        exports.oxmysql:execute('SELECT * FROM apartments WHERE citizenid=@citizenid', {['@citizenid'] = Player.PlayerData.citizenid}, function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
XZCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = XZCore.Functions.GetPlayer(src)
    if Player ~= nil then
        exports.oxmysql:execute('SELECT * FROM apartments WHERE citizenid=@citizenid', {['@citizenid'] = Player.PlayerData.citizenid}, function(result)
...
RegisterServerEvent('xz-apartments:server:SetInsideMeta')
AddEventHandler('xz-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = XZCore.Functions.GetPlayer(src)
...
RegisterNetEvent("xz-apartments:enterBucket", function()
    source = tonumber(source)
    SetPlayerRoutingBucket(source, source)
end)

RegisterNetEvent("xz-apartments:returnBucket", function()
