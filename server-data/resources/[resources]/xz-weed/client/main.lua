...
RegisterNetEvent('xz-weed:client:getHousePlants')
AddEventHandler('xz-weed:client:getHousePlants', function(house)    
    XZCore.Functions.TriggerCallback('xz-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
        ...
        if not plantSpawned then
            for k, v in pairs(housePlants[currentHouse]) do
                local plantData = {
                    ...
                    ["plantProp"] = GetHashKey(XZWeed.Plants[housePlants[currentHouse][k].sort]["stages"][housePlants[currentHouse][k].stage]),
                }
    ...
    Citizen.CreateThread(function()
        if plantSpawned then
            for k, v in pairs(housePlants[currentHouse]) do
                ...
                for _, stage in pairs(XZWeed.Plants[housePlants[currentHouse][k].sort]["stages"]) do
                ...
                for k, v in pairs(housePlants[currentHouse]) do
                    ...
                    local plantData = {
                        ...
                        ["plantSort"] = {
                            ...
                            ["label"] = XZWeed.Plants[housePlants[currentHouse][k].sort]["label"],
                        },
                        ["plantStats"] = {
                            ...
                            ["highestStage"] = XZWeed.Plants[housePlants[currentHouse][k].sort]["highestStage"],
                            ["gender"] = gender,
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    XZCore.Functions.Progressbar("remove_weed_plant", "Harvesting Plant", 8000, false, true, {
                                        ...
                                        TriggerServerEvent('xz-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                                        XZCore.Functions.Notify("Process Canceled", "error")
                                    end)
                                end
                            end
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                XZCore.Functions.Progressbar("remove_weed_plant", "Removing The Plant", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('xz-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
                                    XZCore.Functions.Notify("Process Canceled", "error")
                                end)
...
RegisterNetEvent('xz-weed:client:leaveHouse')
AddEventHandler('xz-weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('xz-weed:client:refreshHousePlants')
AddEventHandler('xz-weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('xz-weed:client:refreshPlantStats')
AddEventHandler('xz-weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(500, function()
            XZCore.Functions.TriggerCallback('xz-weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('xz-weed:client:placePlant')
AddEventHandler('xz-weed:client:placePlant', function(type, item)
    local ped = PlayerPedId()
    local plyCoords = GetOffsetFromEntityInWorldCoords(ped, 0, 0.75, 0)
    local plantData = {
        ["plantCoords"] = {["x"] = plyCoords.x, ["y"] = plyCoords.y, ["z"] = plyCoords.z},
        ["plantModel"] = XZWeed.Plants[type]["stages"]["stage-a"],
        ["plantLabel"] = XZWeed.Plants[type]["label"]
    }
    local ClosestPlant = 0
    for k, v in pairs(XZWeed.Props) do
        if ClosestPlant == 0 then
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            XZCore.Functions.Progressbar("plant_weed_plant", "Planting", 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                ClearPedTasks(ped)            
                TriggerServerEvent('xz-weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
                TriggerServerEvent('xz-weed:server:removeSeed', item.slot, type)
            end, function() -- Cancel
                ClearPedTasks(ped)
                XZCore.Functions.Notify("Process Canceled", "error")
            end)
        else
            XZCore.Functions.Notify("Can't Place Here", 'error', 3500)
        end
    else
        XZCore.Functions.Notify("It's Not Safe Here, try your house", 'error', 3500)
    end
end)

RegisterNetEvent('xz-weed:client:foodPlant')
AddEventHandler('xz-weed:client:foodPlant', function(item)
    local plantData = {}
    if currentHouse ~= nil then
        if ClosestTarget ~= 0 then
            ...
            plantData = {
                ...
                ["plantProp"] = GetHashKey(XZWeed.Plants[housePlants[currentHouse][ClosestTarget].sort]["stages"][housePlants[currentHouse][ClosestTarget].stage]),
                ["plantSort"] = {
                    ["name"] = housePlants[currentHouse][ClosestTarget].sort,
                    ["label"] = XZWeed.Plants[housePlants[currentHouse][ClosestTarget].sort]["label"],
                },
                ["plantStats"] = {
                    ...
                    ["highestStage"] = XZWeed.Plants[housePlants[currentHouse][ClosestTarget].sort]["highestStage"],
                    ["gender"] = gender,
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    XZCore.Functions.Notify('The Plant Does Not Need Nutrition', 'error', 3500)
                else
                    XZCore.Functions.Progressbar("plant_weed_plant", "Feeding Plant", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        TriggerServerEvent('xz-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                    end, function() -- Cancel
                        ClearPedTasks(ped)
                        XZCore.Functions.Notify("Process Canceled", "error")
                    end)
                ...
                XZCore.Functions.Notify("Can't Place Here", "error")
            end
        else
            XZCore.Functions.Notify("Can't Place Here", "error")
        end
