...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Wallet: "+CryptoData.Portfolio+" XZbit('s)");
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (XZ.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://xz-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    XZ.Phone.Data.PlayerData.money.bank = parseInt(XZ.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    XZ.Phone.Notifications.Add("fas fa-university", "Bank", "&#36; "+Price+",- has been withdrawn from your balance!", "#badc58", 2500);
                } else {
                    XZ.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
                }
            });
        } else {
            XZ.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
        ...
        XZ.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill out all fields!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://xz-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    XZ.Phone.Data.PlayerData.money.bank = parseInt(XZ.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    XZ.Phone.Notifications.Add("fas fa-university", "Bank", "&#36; "+Price+",- has been added to your balance!", "#badc58", 2500);
                } else {
                    XZ.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough XZbits..", "#badc58", 1500);
                }
            });
        } else {
            XZ.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough XZbits..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://xz-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        XZ.Phone.Notifications.Add("fas fa-university", "Crypto", "this Wallet-ID doesn't exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        XZ.Phone.Notifications.Add("fas fa-university", "Crypto", "You transferred "+Coins+",- to "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                XZ.Phone.Notifications.Add("fas fa-university", "Crypto", "You can't transfer to yourself..", "#badc58", 2500);
            }
        } else {
            XZ.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough XZbits..", "#badc58", 1500);
        }
