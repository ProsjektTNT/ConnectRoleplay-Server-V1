...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    XZ.Phone.Functions.SetupChatMessages(ChatData);

    $.post('https://xz-phone/ClearAlerts', JSON.stringify({
...
XZ.Phone.Functions.GetLastMessage = function(messages) {
...
XZ.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = XZ.Phone.Functions.GetLastMessage(chat.messages);
...
XZ.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('https://xz-phone/SendMessage', JSON.stringify({
        ...
        XZ.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
// XZ.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('https://xz-phone/GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== XZ.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://xz-phone/SharedLocation', JSON.stringify({
