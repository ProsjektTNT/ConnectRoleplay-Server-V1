...
$(document).on('click', '.bank-app-account', function(e){
    ...
    XZ.Phone.Notifications.Add("fas fa-university", "Bank", "Account number. copied!", "#badc58", 1750);
});
...
XZ.Phone.Functions.DoBankOpen = function() {
    XZ.Phone.Data.PlayerData.money.bank = (XZ.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(XZ.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&#36; "+XZ.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', XZ.Phone.Data.PlayerData.money.bank);
...
$(document).on('click', '.bank-app-account-actions', function(e){
    XZ.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    XZ.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('https://xz-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    XZ.Phone.Notifications.Add("fas fa-university", "Bank", "You have transfered &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    XZ.Phone.Notifications.Add("fas fa-university", "Bank", "You don't have enough balance!", "#badc58", 1500);
                }
                XZ.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        XZ.Phone.Notifications.Add("fas fa-university", "Bank", "Fill out all fields!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://xz-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                XZ.Phone.Notifications.Add("fas fa-university", "Bank", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
XZ.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
...
XZ.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = XZ.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    XZ.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    XZ.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        XZ.Phone.Notifications.Add("fas fa-university", "Bank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    XZ.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});