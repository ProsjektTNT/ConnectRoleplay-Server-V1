...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    XZ.Phone.Data.Applications[AppData.app] = null;

    $.post('https://xz-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('https://xz-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, XZ.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === XZ.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    XZ.Phone.Data.Applications[Installed.data.app] = Installed.data;
