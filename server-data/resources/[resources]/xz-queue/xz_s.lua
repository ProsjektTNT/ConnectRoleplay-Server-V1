...
RegisterServerEvent("xz-queue:playerConnecting")
AddEventHandler("xz-queue:playerConnecting", function(source, reject, d)
    local _source = source
    local currentSteamID, currentDiscordID
    d.defer()
    Wait(50)
    d.update('Handshaking with XZone Queue.')
    Wait(75)
    d.update('Handshaking with XZone Queue..')
    Wait(75)
    d.update('Handshaking with XZone Queue...')
    ...
    TriggerEvent("xz-logs:server:sendLog", {User = _source}, "connects", "A Player Joined The Queue", GetPlayerName(_source) .. " Added to the queue\nHis details are down below", {}, "green", "xz-queue")
	...
	local strr = tostring([[
		{
			...
			"body": [
				{
					"type": "Image",
					"url": "https://cdn.discordapp.com/attachments/844968206526709810/884932442606166056/XZ-PU.png",
					"horizontalAlignment": "Center",
					"width": "70px",
					"spacing": "Small"
				},
