...
RegisterNetEvent("disptach:management", function(state)
    management = state
end)
...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
    if pNotificationData ~= nil and XZCore and XZCore.Functions.GetPlayerData().job then
        if pNotificationData.recipientList then
            for key, value in pairs(pNotificationData.recipientList) do
                if (key == currentJob or management) and value and not disableNotifications and (XZCore.Functions.GetPlayerData().job.onduty or management) then
...
AddEventHandler('dispatch:toggleNotifications', function(state)
    ...
    if state == "on" then
        ...
        TriggerEvent('XZCore:Notify', "Dispatch Enabled.")
    elseif state == "off" then
        disableNotifications = true
        disableNotificationSounds = true
        TriggerEvent('XZCore:Notify', "Dispatch Disabled.", 'error')
    elseif state == "mute" then
        disableNotifications = false
        disableNotificationSounds = true
        TriggerEvent('XZCore:Notify', "Dispatch Muted.")
    else
        TriggerEvent('XZCore:Notify', "You need to type in 'on', 'off' or 'mute'.", 'error')
    end
end)
...
RegisterNetEvent('dispatch:officerDown', function()
    if currentJob == 'police' or management then
...
RegisterNetEvent('dispatch:panicButton', function()
    if currentJob == 'police' or management then
        local playerData = XZCore.Functions.GetPlayerData()
        local callsignal = playerData.metadata["callsign"] ~= nil and playerData.metadata["callsign"] or "None"
		local pos = GetEntityCoords(PlayerPedId(),  true)
...
RegisterNetEvent('dispatch:emsDown', function()
    if currentJob == 'ambulance' or management then
        local playerData = XZCore.Functions.GetPlayerData()
        local callsignal = playerData.metadata["callsign"] ~= nil and playerData.metadata["callsign"] or "None"
		local pos = GetEntityCoords(PlayerPedId(),  true)
		TriggerServerEvent("dispatch:svNotify", {
            ...
            firstStreet = GetStreetAndZone(),
            callSign = callsignal .. ' | ' .. playerData['charinfo']['firstname']:sub(1,1) .. '. ' .. playerData['charinfo']['lastname'],
			cid = XZCore.Functions.GetPlayerData()['cid'],
			origin = {
				x = pos.x,
				y = pos.y,
				z = pos.z
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 57) and (management or currentJob == "police" or currentJob == "ambulance" or currentJob == "reporter") and not XZCore.Functions.GetPlayerData().metadata["isdead"]) then
