...
AddEventHandler("K9:Create", function(dogType)
		...
		XZCore.Functions.Notify("Now in service.", "primary")
...
AddEventHandler("K9:Delete", function()
	...
	DeleteEntity(ldog)
	XZCore.Functions.Notify("Now out of service", "primary")
		...
		if current_dog == 0 then
			...
			if hunting then
				...
				if chance > 10 and distancecheck < 3 and imDead == 0  then
					...
					attacking = false
					following = false
					XZCore.Functions.Notify("The dog smells something very close..", "primary")
					...
					if distancecheck > 120 and imDead == 0  then
						...
						attacking = false
						following = false
						XZCore.Functions.Notify("The dog lost the sent..", "primary")
	...
	if attacking == false then
		if IsEntityAPed(other_ped) then
			if not IsEntityDead(other_ped) then
				...
				hunting = false
				currentState = "A"
				XZCore.Functions.Notify("Dog, Attack!", "primary")
...
AddEventHandler("K9:SniffConfirmedFail", function()
	...
	following = false	
	hunting = false
	XZCore.Functions.Notify("Dog smells nothing!", "primary")
...
AddEventHandler("K9:SniffConfirmed", function()
	...
	if chance > 10 then
		...
		following = false
		hunting = false
		XZCore.Functions.Notify("Dog smells something funny!", "primary")
...
function sit(entity)
	...
	if IsEntityPlayingAnim(entity, animdicstart, animnamestart, 3) then
		...
		TaskPlayAnim(current_dog, animdicstart, animnamestart, 1.0, -1, -1, 2, 0, 0, 0, 0)
		animation_played = "sit"
		XZCore.Functions.Notify("Dog, Sit!", "primary")
...
function laydown(entity)
	...
	if IsEntityPlayingAnim(entity, animdicstart, animnamestart, 3) then
		...
		animation_played = nil
		XZCore.Functions.Notify("Dog, Up!", "primary")
		...
		XZCore.Functions.Notify("Dog, Lay!", "primary")
...
function deathTimer()
    ...
    while imDead == 1 and current_dog ~= 0 do
        ...
        if thecount < 0 then
			...
			XZCore.Functions.Notify("Your dog has been transported to the police vet.", "primary")
