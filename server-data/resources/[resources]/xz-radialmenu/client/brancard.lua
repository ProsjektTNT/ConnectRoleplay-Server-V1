...
RegisterNetEvent('hospital:client:TakeBrancard', function()
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            XZCore.Functions.Notify("Something went wrong my friend..", 'error')
        end
    else
        XZCore.Functions.Notify("Your not near a ambulance..", 'error')
    end
end)

RegisterNetEvent('hospital:client:RemoveBrancard', function()
    local PlayerPed = PlayerPedId()
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(BrancardObject) then
                ...
                TriggerServerEvent('xz-radialmenu:server:RemoveBrancard', PlayerPos, BrancardObject)
                IsAttached = false
                BrancardObject = nil
                IsLayingOnBed = false
            end
        else
            XZCore.Functions.Notify('Your to far away!', 'error')
        end
    end
end)
        ...
        if BrancardObject ~= nil then
            ...
            if Distance <= 1.0 then
                if not IsAttached then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~p~E~w~ -Picking brancard / ~g~H~w~ Locking')
                    ...
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~p~E~w~ - Letting go of the brancard')
                    if IsControlJustPressed(0, 51) then
                        DetachBrancard()
                        IsAttached = false
                    end
...
RegisterNetEvent('xz-radialmenu:client:RemoveBrancardFromArea', function(PlayerPos, BObject)
    local Ped = PlayerPedId()
...
function LayOnBrancard()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('xz-radialmenu:Brancard:BusyCheck', GetPlayerServerId(player), "lay")
        else
            LoadAnim(inBedDicts)
            if Object ~= nil or Object ~= 0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
RegisterNetEvent('xz-radialmenu:Brancard:client:BusyCheck', function(OtherId, type)
    local ped = PlayerPedId()
    if type == "lay" then
        ...
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('xz-radialmenu:server:BusyResult', true, OtherId, type)
        else
            TriggerServerEvent('xz-radialmenu:server:BusyResult', false, OtherId, type)
        end
    else
...
RegisterNetEvent('xz-radialmenu:client:Result', function(IsBusy, type)
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            XZCore.Functions.Notify("Deze brancard is al in gebruik!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
            ...
            XZCore.Functions.Notify("This brancard is already in use!", "error")
            IsAttached = false
        end
    end
end)
...
function AttachToBrancard()
    ...
    if BrancardObject ~= nil then
        if ClosestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('xz-radialmenu:Brancard:BusyCheck', GetPlayerServerId(ClosestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(BrancardObject)
                LoadAnim("anim@heists@box_carry@")
                TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
