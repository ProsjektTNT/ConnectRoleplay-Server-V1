...
RegisterNetEvent('xz-kidnapping:client:SetKidnapping')
AddEventHandler('xz-kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)
...
RegisterNetEvent('xz-trunk:client:KidnapTrunk')
AddEventHandler('xz-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Citizen.Wait(50)
                TriggerServerEvent("xz-trunk:server:KidnapTrunk", GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
            XZCore.Functions.Notify('You didn\'t kidnap this person!', 'error')
        end
    end
end)

RegisterNetEvent('xz-trunk:client:KidnapGetIn')
AddEventHandler('xz-trunk:client:KidnapGetIn', function(veh)
    local ped = PlayerPedId()
    ...
    if TrunkClasses[vehClass].allowed then
        XZCore.Functions.TriggerCallback('xz-trunk:server:getTrunkBusy', function(isBusy)
                ...
                if not inTrunk then
                    if not isBusy then
                        if not isKidnapped then
                            ...
                            TriggerServerEvent('xz-trunk:server:setTrunkBusy', plate, true)
                            inTrunk = true
                            Citizen.Wait(500)
                            SetVehicleDoorShut(closestVehicle, 5, false)
                            XZCore.Functions.Notify('You are in the trunk', 'success', 4000)
                            TrunkCam(true)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('xz-smallresources:trunk:server:setTrunkBusy', plate, nil)
                                SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                                SetEntityCollision(PlayerPedId(), true, true)
                                TrunkCam(false)
                            else
                                XZCore.Functions.Notify('Is the trunk closed?', 'error', 2500)
                            end
                        end
                    else
                        XZCore.Functions.Notify('Already see someone in it?', 'error', 2500)
                    end 
                else
                    XZCore.Functions.Notify('You are already in the trunk', 'error', 2500)
                end 
            else
                XZCore.Functions.Notify('You cannot put this vehicle in the trunk', 'error', 2500)
            end
        end, plate)
    else
        XZCore.Functions.Notify('You cannot put this vehicle in the trunk', 'error', 2500)
...
RegisterNetEvent('xz-trunk:client:GetIn')
AddEventHandler('xz-trunk:client:GetIn', function(isKidnapped)
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    ...
    if closestVehicle ~= 0 then
                ...
                if not disabledCarCheck(closestVehicle) then
                    if not inTrunk then
                        if not isBusy then
                            ...
                            XZCore.Functions.Notify('Anyone in there yet?', 'error', 2500)
                        end 
                    else
                        XZCore.Functions.Notify('You are already in the trunk', 'error', 2500)
                    end 
                else
                    XZCore.Functions.Notify('You can\'t put this vehicle in the trunk ..', 'error', 2500)
                end
            end, plate)
        else
            XZCore.Functions.Notify('You can\'t put this vehicle in the trunk ..', 'error', 2500)
        ...
        XZCore.Functions.Notify('No vehicle to be seen ..', 'error', 2500)
    end
end)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('xz-trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                            SetEntityCollision(PlayerPedId(), true, true)
                            TrunkCam(false)
                        else
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('xz-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('xz-radialmenu:trunk:server:Door', true, plate, 5)
                            else
