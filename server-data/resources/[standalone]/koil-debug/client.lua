...
RegisterNetEvent("koil-debug:toggle", function()
	dickheaddebug = not dickheaddebug
    TriggerEvent("xz-hud:debugState", dickheaddebug)
end)
    ...
    while true do 
        ...
        if dickheaddebug then
            ...
            if IsControlJustReleased(0, 38) then
                if inFreeze then
                    inFreeze = false
                    XZCore.Functions.Notify('Freeze Disabled', 'error')     
                else
                    inFreeze = true             
                    XZCore.Functions.Notify('Freeze Enabled')               
                end
            end
            ...
            if IsControlJustReleased(0, 47) then
                if lowGrav then
                    lowGrav = false
                    XZCore.Functions.Notify('Low Grav Disabled', 'error')             
                else
                    lowGrav = true              
                    XZCore.Functions.Notify('Low Grav Enabled', 'error')               
                end
            end
