...
Citizen.CreateThread(function()
	while true do
		...
		if pumpDistance < 2.5 then
			...
			currentCash = XZCore.Functions.GetPlayerData().money['cash']
		else
			isNearPump = false
				...
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								TriggerServerEvent('XZCore:Server:AddItem', "weapon_petrolcan", 1)
								TriggerEvent("inventory:client:ItemBox", XZCore.Shared.Items["weapon_petrolcan"], "add")
