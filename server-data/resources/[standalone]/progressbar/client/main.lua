...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("process:state", true)
            ...
            XZCore.Functions.Notify("You are already doing something!", "error")
        end
    else
        XZCore.Functions.Notify("Unable to take action!", "error")
    end
end
...
function Cancel()
    TriggerEvent("process:state", false)
    isDoingAction = false
    wasCancelled = true
